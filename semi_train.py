...
import torch
from dataloaders.semi_dataset import (data_process,  Resize_2d,
...
# label and unlabel
parser.add_argument('--paired_bs', type=int, default=1,
...
def train(args):
    ...
    if args.multi_gpu:
        ...
        #train_batch_sampler = BatchSampler(db_train, batch_size, drop_last=False)
        train_batch_sampler = torch.utils.data.RandomSampler(db_train)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            paired_3T_batch, paired_7T_batch = sampled_batch['paired_3T'], sampled_batch['paired_7T']
            unpaired_3T_batch = sampled_batch['unpaired_3T']
            print('paired_3T_batch.shape',paired_3T_batch.shape)
            print('paired_7T_batch.shape',paired_7T_batch.shape)
            print('unpaired_3T_batch.shape',unpaired_3T_batch.shape)
            paired_7T_batch = torch.unsqueeze(paired_7T_batch, 1)
            paired_3T_batch, paired_7T_batch = paired_3T_batch.cuda(), paired_7T_batch.cuda()
            unpaired_3T_batch= unpaired_3T_batch.cuda()
            
            noise = torch.clamp(torch.randn_like(
               unpaired_3T_batch) * 0.1, -0.2, 0.2)

            ema_inputs_n1 = unpaired_3T_batch + noise
            ema_inputs_n2 = ptb_f(unpaired_3T_batch)
            print('ema_inputs_n1.shape', ema_inputs_n1.shape)
            print('ema_inputs_n2.shape', ema_inputs_n2.shape)

            model.train()
            outputs = model(paired_3T_batch)
            unp_outputs = model(unpaired_3T_batch)

            with torch.no_grad():
               ema_output_n1 = ema_model(ema_inputs_n1)
               ema_output_n2 = ema_model(ema_inputs_n2)

            supervised_loss = criterion_L1(torch.squeeze(outputs), torch.squeeze(paired_7T_batch))

            consistency_weight = get_current_consistency_weight(iter_num//150)

            consistency_loss_1 = torch.mean(
                (unp_outputs - ema_output_n1) ** 2)
            consistency_loss_2 = torch.mean(
               (unp_outputs - ema_output_n2) ** 2)
            ...
            if iter_num % 10 == 0:
                image = paired_3T_batch[0, 0:1, :, :].permute(
                    0, 1, 2).repeat(3, 1, 1)
                ...
                image = paired_7T_batch[0, 0:1, :, :].permute(
                    0, 1, 2).repeat(3, 1, 1)
