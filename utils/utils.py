...
def Generator_3D_patches(path_patients,batchsize, inputKey='dataMR',outputKey='dataCT'):
    ...
    while True:
        ...
        for idx,namepatient in enumerate(patients):
            ...
            dataMR=np.squeeze(dataMR,axis=1)
            dataCT_ori = dataCT


            dataCT=np.squeeze(dataCT,axis=1)
            #print('mr source shape h5 ',dataMR.shape)
            #print('mr target shape h5 ',dataCT.shape)
...
def evaluate_res(patch_MR, netG, nd=2):
    patch_MR = torch.from_numpy(patch_MR)

    patch_MR = patch_MR.unsqueeze(0)
    #print('shape of patch_MR ',patch_MR.shape)
    ...
    tmp = res
    patchOut = tmp.data.cpu().numpy().squeeze()
    #print('shape of patchOut ',patchOut.shape)
...
'''
def testOneSubject_aver_res(MR_image,MR_patch_sz,CT_patch_sz,step, netG, nd=2):
    ...
    eps = 1e-5
    [row,col,leng] = matFA.shape     #[182, 218, 182]
    ...
    matOut = np.zeros((matFAOut.shape[0],matFAOut.shape[1],matFAOut.shape[2]))
    used = np.zeros((matFAOut.shape[0],matFAOut.shape[1],matFAOut.shape[2]))+eps
    ...
    for i in range(0,row-dSeg[0]+1,step[0]):
        for j in range(0,col-dSeg[1]+1,step[1]):
            for k in range(0,leng-dSeg[2]+1,step[2]):
                ...
                temppremat = evaluate_res(volFA, netG, nd=nd)
                #print 'max: ',np.amax(temppremat),'min: ',np.amin(temppremat)
                #print('temppremat shape : ',temppremat.shape)
